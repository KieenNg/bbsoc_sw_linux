...
int main(){
    ...
    //tao iio context (tao 1 phien lam viec)
    ...
    tx_device = iio_context_find_device(ctx, "axi-adrv9002-tx-lpc");
    if(!tx_device){
        fprintf(stderr, "Loi: Khong tim thay thiet bi axi-adrv9002-tx-lpc.\n");
        return -1;
    }
    //Tim 4 channel
    ...
    //tao buffer cyclic
    struct iio_buffer *buf = iio_device_create_buffer(tx_device, SAMPLES_COUNT, true);
    int i = 0;
    for(int16_t *ptr = iio_buffer_start(buf); ptr < (int16_t*)iio_buffer_end(buf); ptr += iio_buffer_step(buf)/2){
        *ptr = 32767 - sine_lut[i];
        i = (i+1) % SAMPLES_COUNT;
    }
    //day du lieu di
    iio_buffer_push(buf);
	...
	getchar();//khong duoc huy buffer hay context ngay lap tuc trong khi buffer dang push
