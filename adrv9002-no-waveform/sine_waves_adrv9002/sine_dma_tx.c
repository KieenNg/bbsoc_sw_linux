...
// Mảng chứa dữ liệu sóng sin
int16_t sine_lut[SAMPLES_COUNT] = {
...
int main(int argc, char **argv) {
    // 1. Tạo Context (kết nối tới thiết bị)
    struct iio_context *ctx = iio_create_default_context();
    if (!ctx) {
        fprintf(stderr, "Lỗi: Không tạo được context.\n");
        return -1;
    }

    // 2. Tìm thiết bị phát (TX)
    struct iio_device *tx = iio_context_find_device(ctx, "axi-adrv9002-tx-lpc");
    if (!tx) {
        fprintf(stderr, "Lỗi: Không tìm thấy thiết bị TX.\n");
        iio_context_destroy(ctx);
        return -1;
    }

    // 3. Tìm và Kích hoạt (Enable) CẢ 4 KÊNH
    // Trong chế độ MIMO, phải bật cả 4 để giữ đúng cấu trúc dữ liệu 64-bit
    struct iio_channel *ch_i0 = iio_device_find_channel(tx, "voltage0", true);
    struct iio_channel *ch_q0 = iio_device_find_channel(tx, "voltage1", true);
    struct iio_channel *ch_i1 = iio_device_find_channel(tx, "voltage2", true);
    struct iio_channel *ch_q1 = iio_device_find_channel(tx, "voltage3", true);

    if (!ch_i0 || !ch_q0 || !ch_i1 || !ch_q1) {
        fprintf(stderr, "Lỗi: Không tìm thấy đủ các kênh.\n");
        iio_context_destroy(ctx);
        return -1;
    }
    ...
    // 4. Tạo Buffer Cyclic (Phát lặp lại liên tục)
    struct iio_buffer *buf = iio_device_create_buffer(tx, SAMPLES_COUNT, true);
    if (!buf) {
        perror("Lỗi: Không tạo được buffer");
        iio_context_destroy(ctx);
        return -1;
    }

    // 5. Điền dữ liệu vào Buffer (Quan trọng: Sửa lỗi tại đây)
    ptrdiff_t step = iio_buffer_step(buf); 
    void *start = iio_buffer_start(buf);
    void *end = iio_buffer_end(buf);

    int i = 0;
    for (char *ptr = (char *)start; ptr < (char *)end; ptr += step) {
        int16_t *sample_ptr = (int16_t *)ptr;
        
        sample_ptr[0] = 32767 - sine_lut[i]; // Ghi giá trị từ bảng sin vào kênh I0
        sample_ptr[1] = 0;           // Kênh Q0
        sample_ptr[2] = 0;           // Kênh I1
        sample_ptr[3] = 0;           // Kênh Q1

        i = (i + 1) % SAMPLES_COUNT;
    }

    // 6. Đẩy dữ liệu xuống phần cứng
    printf("Dang phat song sin tren kenh I0 (Tx1_I)...\n");
    int ret = iio_buffer_push(buf);
    if (ret < 0) {
        fprintf(stderr, "Lỗi push buffer: %d\n", ret);
        iio_buffer_destroy(buf);
        iio_context_destroy(ctx);
        return -1;
    }

    // 7. Giữ chương trình chạy (vì buffer là cyclic, DMA sẽ tự phát lặp lại)
    while (1) {
        sleep(1); 
    }

    // Dọn dẹp (thực tế code sẽ không chạy đến đây nếu dùng while(1))
    iio_buffer_destroy(buf);
    iio_context_destroy(ctx);
    return 0;
}